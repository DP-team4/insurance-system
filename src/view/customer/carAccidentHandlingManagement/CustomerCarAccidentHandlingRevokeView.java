...
public class CustomerCarAccidentHandlingRevokeView {
	...
	public void show(CustomerCarAccidentHandlingListView customerCarAccidentHandlingListView, Customer customer) {
		try {
			...
			// 사고처리 신청 취소 화면(사고처리 신청 아이디 입력창  + '확인', '취소' 버튼)을 보여준다
			System.out.println("\n[ 사고처리 신청 취소 ]");
			if(customerCarAccidentHandlingService.getByCustomerId(customer.getId()).size() == 0) {
				System.out.println("신청 내역이 없어 사고처리 신청 취소가 불가합니다. 이전 화면으로 돌아갑니다."); return;
			}
			System.out.println("사고처리 신청 목록을 조회하려면 (list)");
			boolean showList = true;
			String id = "";
			while(showList) {
		        System.out.print("취소할 사고처리 신청 아이디 >> "); id = getInputAndCheckInvalid();
		        if(id.equals("list")) {
	        ...
	        System.out.print("\n사고처리 신청 취소를 진행하시겠습니까? 예(1) 아니오(기타) >> "); String input = scanner.nextLine().trim();
	        if(input.equals("1")) {
	            // A1. 사고처리 신청 취소 양식을 덜 입력한 경우
